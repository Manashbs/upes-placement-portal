...
export const SPRRotationView: React.FC = () => {
  const { sprs, dutyAssignments, addSpr, deleteSpr, exportSPRDutiesReport } = usePortal();
    ...
    <div className="space-y-8 select-none pb-12">
      ...
      <div className="flex items-center justify-between flex-wrap gap-4 border-b border-slate-200/60 pb-6">
        <div className="flex items-center space-x-3 flex-wrap gap-2">
          ...
          <button
            onClick={() => exportSPRDutiesReport()}
            className="inline-flex items-center space-x-2 bg-emerald-50 hover:bg-emerald-100 text-emerald-900 border border-emerald-300/80 text-sm font-extrabold px-5 py-3 rounded-2xl shadow-xs transition-all active:scale-95 cursor-pointer"
          >
            <FileSpreadsheet className="w-4 h-4 text-emerald-600" />
            <span>Export SPR Duty Roster (.xlsx)</span>
          </button>
        </div>
            ...
            <div className="flex items-center justify-between px-6 py-4 border-t border-slate-100 shrink-0 bg-slate-50/50">
              <button
                onClick={() => {
                  ...
                  exportSPRDutiesExcel(sprDuties, sprs, `${selectedSprDetails.name}_Duties`);
                }}
                className="inline-flex items-center space-x-2 bg-emerald-50 hover:bg-emerald-100 text-emerald-900 border border-emerald-300/80 text-xs font-bold px-4 py-2.5 rounded-xl transition-all cursor-pointer"
              >
                <FileSpreadsheet className="w-4 h-4 text-emerald-600" />
                <span>Download {selectedSprDetails.name}'s Duties (.xlsx)</span>
              </button>
