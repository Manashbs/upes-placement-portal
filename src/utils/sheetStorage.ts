 ...
 * Strictly queries roundId if provided, without leaking sheets from other rounds.
 */
 ...
 * Strictly checks the requested roundId and never falls back to another round's sheet.
 */
export function getSheetMetaSync(roundId?: string): { headers: string[]; rows: any[][]; fileName?: string } | undefined {
  ...
  if (roundId) {
    if (window.__upes_excel_meta?.has(roundId)) {
      return window.__upes_excel_meta.get(roundId);
    }
    try {
      const raw = localStorage.getItem(`upes_excel_raw_${roundId}`);
      if (raw) return JSON.parse(raw);
    } catch {}
    // If roundId was specified and not found, strictly return undefined
    return undefined;
  }
  ...
  try {
    const raw = localStorage.getItem('upes_excel_raw_LATEST');
    if (raw) return JSON.parse(raw);
  } catch {}
...
/**
 * Checks if an Excel shortlist sheet has been uploaded for a specific roundId.
 */
export function hasSheetForRound(roundId?: string): boolean {
  if (!roundId) return false;
  ensureGlobalMaps();
  if (typeof window !== 'undefined') {
    if (window.__upes_excel_buffers?.has(roundId)) return true;
    if (window.__upes_excel_meta?.has(roundId)) return true;
  }
  try {
    if (localStorage.getItem(`upes_excel_raw_${roundId}`)) return true;
  } catch {}
  return false;
}
 ...
 * Strictly searches for roundId if specified without cross-round fallback.
 */
    ...
    return new Promise((resolve) => {
      ...
      req.onsuccess = () => {
        if (req.result) {
          ...
          // If roundId was explicitly passed and not found, DO NOT bleed LATEST
          if (roundId) {
            resolve(undefined);
            return;
          }
