...
import { RoundStudent, Student, SPRDutyAssignment, SPR } from '../types';
...
export function exportSPRDutiesExcel(
  dutyAssignments: SPRDutyAssignment[],
  allSprs: SPR[],
  title: string = 'SPR_Duty_Allocation_Report',
  format: 'xlsx' | 'csv' = 'xlsx'
) {
  const sprMap = new Map<string, SPR>();
  allSprs.forEach((s) => sprMap.set(s.id, s));

  const exportData = dutyAssignments.map((duty, index) => {
    const sprInfo = sprMap.get(duty.sprId);
    return {
      'S.No': index + 1,
      'SPR Name': duty.sprName,
      'SAP ID': sprInfo?.sapId || 'N/A',
      'Branch': sprInfo?.branch || 'N/A',
      'Contact Email': sprInfo?.email || 'N/A',
      'Contact Phone': sprInfo?.phone || 'N/A',
      'Assigned Company': duty.companyName,
      'Round Title': duty.roundName,
      'Process Date': duty.date,
      'Time Window': duty.timeWindow,
      'Allotted Venue': duty.venue,
      'Role / Duty': duty.role,
      'Duty Status': duty.status,
      'Assigned At': duty.assignedAt ? new Date(duty.assignedAt).toLocaleString() : 'N/A',
    };
  });

  const worksheet = XLSX.utils.json_to_sheet(exportData);
  const workbook = XLSX.utils.book_new();
  XLSX.utils.book_append_sheet(workbook, worksheet, 'SPR Duties');

  const ext = format === 'csv' ? 'csv' : 'xlsx';
  const fileName = `UPES_${title.replace(/\s+/g, '_')}_${new Date().toISOString().split('T')[0]}.${ext}`;
  XLSX.writeFile(workbook, fileName, { bookType: ext as any });
}
